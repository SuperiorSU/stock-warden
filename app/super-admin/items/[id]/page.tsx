...
export default function SuperAdminItemDetailsPage() {
  ...
  return (
    <div className="space-y-6 page-enter">
      <Link href="/super-admin/items" className="inline-flex items-center space-x-2 text-sm font-medium text-[--ink-secondary] hover:text-[--ink-primary]">
        <ArrowLeft size={16} />
        Back to Items
      </Link>
                ...
                records.map((r) => (
                  <tr key={r.id} className="hover:bg-[--bg-canvas] transition-colors">
                    <td className="px-5 py-3 font-medium text-[--ink-primary]">
                      <Link href={`/super-admin/employees/${r.userId}`} className="hover:underline">{r.employeeName}</Link>
                    </td>
